...
def main(model_name=None, finetune=False, syn_file='syn_diffG.pkl', 
    ...
    if args.data == 'CRASH':
        ...
        if True: #TODO: compare preprocessing performance difference
            ...
            # generate averaged adj
            avg_adj = [np.zeros_like(adj_mx[0][i]) for i in range(len(adj_mx[0]))]
            for i in range(len(adj_mx)):
                for j in range(len(adj_mx[0])):
                    avg_adj[j] += adj_mx[i][j]

            for i in range(len(adj_mx[0])):
                avg_adj[i] /= len(adj_mx)
    ...
    if args.data == 'CRASH':
        ...
        #### use diffG model
        # supports = {}
        # supports['train'] = avg_adj
        # supports['val'] = avg_adj
        # supports['test'] = avg_adj
        ...
        ### use sameG model
        supports = [torch.tensor(i).to(device) for i in avg_adj]
        ...
        if model_name is None or finetune is True:
            ...
            for i in range(1,args.epochs+1):
                ...
                train_rmse = []
                t1 = time.time()
                # engine.set_state('train') ### for diffG model
                ...
                for _train_idx in train_idx:
                    ...
                    y_F = fmri_mat[subj_id, F_idxer, :][offset:][
                                batch_i * args.batch_size: (batch_i + 1) * args.batch_size]
                    y_F = torch.Tensor(y_F[...,None])#.transpose(1, 3) ###diffG
                    ...
                    # metrics = engine.train_CRASH(x_F, y_F, y_E, region_assignment, [subj_id]*args.batch_size) ### for diffG model
                    metrics = engine.train_CRASH2(x_F, y_F, y_E)
                ...
                s1 = time.time()
                # engine.set_state('val') ### for diffG model
                ...
                for subj_id in range(nValid):
                    ...
                    for batch_i in range(batch_per_sub):
                        ...
                        if subj_id == 0 and batch_i == 0: # only viz the first one
                            # metrics = engine.eval_CRASH(x_F, y_F, y_E, region_assignment, [subj_id]*args.batch_size, viz=True) ### diffG
                            metrics = engine.eval_CRASH2(x_F, y_F, y_E, viz=False) ### sameG
                        else:
                            # metrics = engine.eval_CRASH(x_F, y_F, y_E, region_assignment, [subj_id]*args.batch_size) ### diffG
                            metrics = engine.eval_CRASH2(x_F, y_F, y_E)
    ...
    if args.data == 'CRASH':
        # engine.set_state('test') ### for diffG
        # engine.set_state('train') # overfit single batch
        real_Fs = []
        real_Es = []
        pred_Fs = []
        ...
        for subj_id in range(nTest):
            ...
            for batch_i in range(batch_per_sub):
                ...
                # metrics = engine.eval_CRASH(x_F, y_F, y_E, region_assignment, [subj_id]*args.batch_size) ###diffG
                metrics = engine.eval_CRASH2(x_F, y_F, y_E, viz=True)
                ...
                if batch_i == 0:
                    if scatter:
                        ...
                        if F_only:
                            ...
                            plt.legend()
                            plt.figure(4)
                            plt.plot(real_Fs[0].squeeze().cpu().numpy()[0,4], label='real Fs')
                            plt.plot(pred_Fs[0].squeeze().cpu().numpy()[0,4], label='pred Fs')
                            plt.legend()
                            plt.figure(9)
                            plt.plot(real_Fs[0].squeeze().cpu().numpy()[0,9], label='real Fs')
                            plt.plot(pred_Fs[0].squeeze().cpu().numpy()[0,9], label='pred Fs')
                            plt.legend()
                            # plt.figure(14)
                            # plt.plot(real_Fs[0].squeeze().cpu().numpy()[0,14], label='real Fs')
                            # plt.plot(pred_Fs[0].squeeze().cpu().numpy()[0,14], label='pred Fs')
                            # plt.legend()
                            plt.show()
                        else:
                            plt.figure()
